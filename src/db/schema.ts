...
export const tweetsTable = pgTable(
  "tweets",
  {
    ...
    startTime: varchar("start_time", { length: 20 }).notNull(), // 新的 startTime 列
    endTime: varchar("end_time", { length: 20 }).notNull(),     // 新的 endTime 列
  },
  (table) => ({
...
export const choicesTable = pgTable(
  ...
  (table) => ({
    userHandleIndex: index("user_handle_index").on(table.userHandle),
    tweetIdIndex: index("tweet_id_index").on(table.tweetId),
    dateHourIndex: index("date_hour_index").on(table.date, table.hour),
    uniqCombination: unique().on(table.userHandle, table.tweetId, table.date, table.hour),
  }),
);
