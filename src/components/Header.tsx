...
export default function Header() {
  ...
  return (
    ...
    <div className="flex justify-between px-6 py-6 bg-green-100">
      <div className="flex gap-2">
        {/* <div className="p-2">
          <Link href="/">
            <Image src={larry} alt="Join Me" width={60} height={60} />
          </Link>
        </div> */}
        <Link href={`/?username=${username}&handle=${handle}`}> {/* 使用 Link 组件包装 Home 按钮 */}
      ...
      <p className="text-sm text-gray-500" style={{ display: 'none' }}>{`@${handle}`}</p>
      </div>
      <p className="text-xl font-bold px-4 flex items-center justify-center text-green-700">{username ?? "..."}</p>
      <ProfileButton />
    </div>
  );
}
  ...
  return (
    <button className="group w-full">
      <div
        ...
        className="flex w-fit items-center gap-4 rounded-full p-2 transition-colors duration-300 group-hover:bg-white lg:pr-4"
      >
