...
export default async function Home({
  ...
  const tweetIdResult = await db
    ...
    .limit(1)
    .execute();
  if (tweetIdResult.length > 0) {
    NewtweetId = tweetIdResult[0].id + 1;
    // console.log(NewtweetId);
  } else {
    NewtweetId = 1;
    // Handle the case where no rows were returned, e.g., set a default value
    ...
    <>
      <div className="flex h-screen w-full flex-col overflow-scroll pt-2 px-6">
        ...
        <div className="px-4 pt-2 flex justify-end">
          <TweetInput NewtweetId={NewtweetId}/>
        </div>
        {/* <Separator /> */}
        <div className="px-4 pt-1">
          <SearchTweets tweets={tweets} username={username} handle={handle} />
        </div>
        {/* {tweets
          .filter((tweet) => tweet.content.includes(searchKeyword))
          .map((tweet) => (
          <Tweet
            key={tweet.id}
            id={tweet.id}
